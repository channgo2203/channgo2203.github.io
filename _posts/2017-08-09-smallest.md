...
The key idea of the _array-based_ and _divide and conquer_ solutions is that not every natural number in the range `[0...len]` where `len` is the length of the input list `X` can be in `X`.
