...
If we have `u1`, `u2`, `v1`, and `v2` such that `u1 \\ v2 = u1` and `u2 \\ v1 = u2`. In other words, `u1` and `v2` (respectively `u2` and `v1`) are disjoint. Them following the properties above, we get
