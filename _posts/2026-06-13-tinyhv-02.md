...
Recall from previous part, the name for privilege in ARMv8 is Exception level, often abbreviated to EL. The exception levels are numbered, normally abbreviated and referred to as EL<x>, where <x> is a number between 0 and 3. The higher the level of privilege the higher the number.
...
And the linker script to run with QEMU virtual machine:
